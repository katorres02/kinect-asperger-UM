...
//			case KinectWrapper.Gestures.Jump:
//				gestureData.checkForGestures.Add(KinectWrapper.Gestures.Squat);
//				break;
//				
//			case KinectWrapper.Gestures.Squat:
//				gestureData.checkForGestures.Add(KinectWrapper.Gestures.Jump);
//				break;
//				
//			case KinectWrapper.Gestures.Push:
//				gestureData.checkForGestures.Add(KinectWrapper.Gestures.Pull);
//				break;
//				
//			case KinectWrapper.Gestures.Pull:
//				gestureData.checkForGestures.Add(KinectWrapper.Gestures.Push);
//				break;
		}
	...
	public bool IsGestureComplete(uint UserId, KinectWrapper.Gestures gesture, bool bResetOnComplete)
	{
		int index = GetGestureIndex(UserId, gesture);
	...
	void Start()
	{
		...
		if(ComputeColorMap)
		{
			...
			//usersClrRect = new Rect(100, 100, 128, 128);
...
//			KinectWrapper.UpdateKinectSensor();
			...
			if(Player1Calibrated)
			{
				...
				foreach(KinectWrapper.GestureData gestureData in player1Gestures)
				{
					...
					{
						if(gestureData.gesture == KinectWrapper.Gestures.Click)
						{
					...
					{
						if((gestureData.gesture == KinectWrapper.Gestures.RightHandCursor || 
							gestureData.gesture == KinectWrapper.Gestures.LeftHandCursor) && 
						...
						for(int j = 0; j < KinectWrapper.Constants.NuiSkeletonCount; j++)
						{
							if(j != i)
							{
								...
								if((skeletonDataOther.eTrackingState == KinectWrapper.NuiSkeletonTrackingState.SkeletonTracked) &&
									(kinectToWorld.MultiplyPoint3x4(skeletonDataOther.Position).z < skeletonPos.z))
								{
									bClosestUser = false;
									break;
								}
